...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    //threadlocal中载荷信息 userId userKey
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (StringUtils.isEmpty(userKey)){ //判断是存在user Key，不存在就生成

            userKey = UUID.randomUUID().toString();

            CookieUtils.setCookie(request,response,this.jwtProperties.getUserKeyName(),userKey,jwtProperties.getExpier());
        }

        //request.setAttribute("userKey",userKey);//放入request域中，方便后面使用
        userInfo.setUserKey(userKey);

        if (StringUtils.isEmpty(token)){
            //如果用户没有登陆，直接返回一个 游客userKey 并直接返回，不再执行后面业务
