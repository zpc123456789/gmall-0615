...
public class IndexController {
    ...
    @GetMapping("cates")
    public Resp<List<CategoryEntity>> queryLevel1Catrgory(){
       List<CategoryEntity> categoryEntities = this.indexService.queryLevel1Catrgory();
    ...
    public Resp<List<CategoryVO>> queryCategoryVO(@PathVariable("pid")Long pid){
        ...
        List<CategoryVO> categoryVOS = this.indexService.queryCategoryVOTest(pid);
    ...
    public Resp<Object> testLock(HttpServletRequest request){
        ...
        System.out.println(request.getLocalPort());//获取端口号

        String msg = this.indexService.testLock();
        return Resp.ok(msg);
    }

    @GetMapping("redissontLock")
    public Resp<Object> redissontLock(HttpServletRequest request){
